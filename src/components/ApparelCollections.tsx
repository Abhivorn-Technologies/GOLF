import React from 'react';
import Link from 'next/link';

export default function ApparelCollections() {
  return (
    <section className="bg-white py-[64px] border-b border-[#c1c9bf]">
      <div className="max-w-[1280px] mx-auto px-[64px]">
        {/* Header */}
        <div className="flex items-center justify-between mb-[40px]">
          <h2 className="text-[36px] font-serif font-semibold text-[#1b1c1c] leading-none">Featured Collections</h2>
          <Link href="#" className="flex items-center gap-1 group text-[#414942] hover:text-green-700 transition-colors">
             <span className="text-[14px] font-serif font-medium uppercase tracking-[1.4px]">View All</span>
             <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="ml-1 group-hover:translate-x-1 transition-transform"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
          </Link>
        </div>

        {/* 2-column Bento Layout */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[24px]">
          
          {/* Card 1: Men's Performance */}
          <Link href="/category/apparel/mens" className="block relative h-[400px] rounded-[16px] overflow-hidden group">
            <div className="absolute inset-0 bg-zinc-200">
               {/* Placeholder for actual image */}
               <div className="w-full h-full bg-[#f4f6f4] flex items-center justify-center group-hover:scale-105 transition-transform duration-700">
                 <span className="text-[#717b71] font-serif italic">Men's Performance Image</span>
               </div>
            </div>
            {/* Gradient Overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
            
            {/* Content */}
            <div className="absolute bottom-[32px] left-[32px] right-[32px] flex items-end justify-between">
              <div>
                <p className="text-[14px] font-serif font-medium text-white/80 uppercase tracking-[1.4px] mb-[8px]">New Arrivals</p>
                <h3 className="text-[32px] font-serif font-semibold text-white leading-none">Men's Performance</h3>
              </div>
              <div className="w-[48px] h-[48px] rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white group-hover:bg-white group-hover:text-[#1b1c1c] transition-colors">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
              </div>
            </div>
          </Link>

          {/* Card 2: Women's Essentials */}
          <Link href="/category/apparel/womens" className="block relative h-[400px] rounded-[16px] overflow-hidden group">
            <div className="absolute inset-0 bg-zinc-200">
               {/* Placeholder for actual image */}
               <div className="w-full h-full bg-[#f4f6f4] flex items-center justify-center group-hover:scale-105 transition-transform duration-700">
                 <span className="text-[#717b71] font-serif italic">Women's Essentials Image</span>
               </div>
            </div>
            {/* Gradient Overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent"></div>
            
            {/* Content */}
            <div className="absolute bottom-[32px] left-[32px] right-[32px] flex items-end justify-between">
              <div>
                <p className="text-[14px] font-serif font-medium text-white/80 uppercase tracking-[1.4px] mb-[8px]">Core Pieces</p>
                <h3 className="text-[32px] font-serif font-semibold text-white leading-none">Women's Essentials</h3>
              </div>
              <div className="w-[48px] h-[48px] rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white group-hover:bg-white group-hover:text-[#1b1c1c] transition-colors">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>
              </div>
            </div>
          </Link>

        </div>
      </div>
    </section>
  );
}
