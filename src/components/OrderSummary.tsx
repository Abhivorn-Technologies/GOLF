          ...
          cartItems.map((item, index) => {
            ...
            return (
              <div key={item.cartItemId} className="flex gap-[16px] items-start">
                <Link href={`/product/${item.product.id}`} className="w-[80px] h-[80px] bg-white border border-[#c4c6cc] rounded-[8px] flex items-center justify-center p-[8px] hover:border-black transition-colors shrink-0">
                  <img 
                ...
                <div className="flex-1 flex flex-col min-w-0">
                  <div className="flex justify-between items-start gap-4">
                    <Link href={`/product/${item.product.id}`} className="font-['Hanken_Grotesk'] font-bold text-[16px] text-black hover:underline hover:text-green-800 transition-colors truncate">
                      {item.product.name}
                    </Link>
