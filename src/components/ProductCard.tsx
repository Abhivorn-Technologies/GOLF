  ...
  return (
    <div className="bg-white border border-gray-100 rounded-[16px] p-[16px] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 group cursor-pointer flex flex-col h-full relative overflow-hidden">
      ...
      <div className="flex flex-col flex-grow">
        <span className="text-[11px] font-bold text-gray-500 uppercase tracking-[2px] mb-[12px] block">
          {product.brand}
        </span>
        <Link href={`/product/${product.slug || product.id}`} className="text-[15px] font-semibold text-zinc-900 group-hover:text-black transition-colors leading-snug mb-[8px]">
          {product.name}
        </Link>
      </div>
      
      {/* Price & Action */}
      <div className="pt-[20px] mt-auto flex justify-between items-end border-t border-gray-50">
        ...
        <Link 
          ...
          className="w-[44px] h-[44px] rounded-full bg-gray-50 flex items-center justify-center text-zinc-900 transition-all duration-300 group-hover:bg-black group-hover:text-white"
        >
